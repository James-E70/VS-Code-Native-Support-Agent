...
- KNOWN FAILURE MODE — CONFIGURATION INFERENCE FROM LOG OR BEHAVIOR: Do not recommend configuring a specific CargoWise field, option, tab column, or grid entry based solely on it appearing in an autorating, workflow, or processing log. Log entries record engine runtime evaluation results — they do not confirm that the referenced value or field exists as a user-configurable UI option. Any configuration step recommended as a fix must be confirmed to exist in the CargoWise UI via a WTA article, a directly reviewed client screenshot, or prior verified product knowledge. Example failure: CS02342347 — autorating log entry "OBILL-UNT-CN-40HC" was incorrectly read as evidence that a container type field exists in Rate Lines and Items for FCL Freight; that field does not exist, and the entry simply recorded which container the engine matched at runtime.
...
- Registry mechanism scope rule: before recommending any registry setting as a fix for a specific error or validation failure, verify via WTA or WI search that the registry's scope reaches the exact stage of the process that produces the error. A registry related to the same feature area is NOT sufficient — the required check is whether it controls the specific validation, calculation, or service task step that generates the failure. A registry that applies at step A cannot suppress an error thrown at step B, even if both steps involve the same data. Do not recommend a registry as a fix until this scope match is confirmed by authoritative documentation.
- Caveat bypass rule: do not recommend a configuration, registry, workaround, or any other solution as a potential fix when it is explicitly documented to apply to a different scenario, entity type, charge type, or condition than the one the client is experiencing — even with a softening caveat such as "test in non-production", "this may also apply", or "worth reviewing". The caveat is a rationalization bypass: it does not make an undocumented recommendation acceptable. The required test before including any solution is: does authoritative documentation (WTA, Update Note, confirmed WI) explicitly confirm this solution applies to the client's specific scenario? If the answer is no or unknown, remove the recommendation entirely.
...
- For every WTA article, Update Note, how-to, FAQ, or guide referenced in the response body: does the sentence containing the reference include the URL inline using the format <title or description> - <url>? If no, add the inline URL before sending. A URL in the footer only does not satisfy this check.
- For every WTA URL cited in the response body: was the document content retrieved and read (via mcp_wtgkb_get-document-by-id) during this investigation? If no, retrieve and read it before finalizing. An article title that sounds relevant is NOT the same as a read article confirming the capability.
- For every registry setting recommended as a fix for a specific error: have I verified via WTA or WI search that this registry controls the exact stage of the process that produces the error — not just the same feature area? If not verified to that level of precision, remove or requalify the recommendation before sending.
- For every solution (configuration, registry, workaround, or other fix) I am recommending: is it explicitly documented for the client's specific scenario, entity type, or charge type? If it is documented for a different scenario and I am softening the recommendation with a phrase like "test in non-production" or "this may also apply" — remove the recommendation entirely. The caveat is a rationalization bypass, not a safe fallback.
