...
After all attachments have been read and the image inventory is complete, and before any WTA hypothesis-forming search begins, write in chat an explicit gate log with the following two entries: (1) ROOT CAUSE WI SEARCH — run `mcp_ediprod_filter-workitems` (or `mcp_wtgkb_search-knowledge-many-results`) using both: (a) the exact error string if one is present in the attachments or incident description, AND (b) the functional failure mode described in plain language (feature/service + what it does wrong). Both search approaches must be attempted. Write the results — including "no relevant WI found" — in chat before forming any hypothesis. (2) CROSS-ORG SIMILAR INCIDENT CHECK — if a relevant WI is found, call `mcp_ediprod_get-job-details` on it and check its attached incidents list for cross-org pattern evidence. If no WI is found, run `mcp_ediprod_filter-incidents` with keywords from the error string or functional description to search for similar reports across all orgs. Write the results in chat. This gate is exploratory — it asks "is this a known issue?" before any investigation direction is chosen. It is distinct from the pre-draft WORK ITEM CHECK (step 3 of the pre-draft audit), which is confirmatory and verifies that planned response claims are consistent with current product behavior. Root cause: CS02416875 (July 2026) — the CargoSphere error "None of the specified contract numbers is a valid carrier contract number" was accepted at face value as a configuration problem; no WI search was run on the error string or functional failure mode; WI01054128 (closed April 2026, CargoSphere rate-limiting fix) and prior incident CS02274233 (same org, same reporter, identical failure) were not found until the second investigation session.
