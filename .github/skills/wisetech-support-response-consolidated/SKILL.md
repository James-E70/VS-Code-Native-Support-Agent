...
- EMBEDDED IMAGES IN OFFICE DOCUMENTS: When a converted Office document (XLSM, XLSX, DOCX, PDF) exposes image links (e.g. `ediprod:///docs/{uuid}/images/NN.png`), every linked image is an independent piece of evidence that must be individually read with the image-viewer tool before the review is considered complete. A file-level tool call on the document does NOT constitute a review of its embedded images. Before any PARSE FAILURES = NONE declaration, list every image link from every converted document and confirm a tool call was made for each one. Root cause: CS02401494 (July 2026) — the XLSM exposed 20 image links across 5 tabs; images from Test 1 tab were not reviewed; UPG logs in those images contradicted the claim that Test 1 was resolved — PARSE FAILURES = NONE was declared without confirming every embedded image had been individually read.
