...
- Before the final chat response to the user, run a mandatory completion check covering:
  ...
  - A list of every external authority URL that was fetched during the investigation (successful or failed)
...
- If any external authority URL fetch failed or returned unreadable content during the investigation, include exactly one line in the final chat response: `EXTERNAL URL COULD NOT BE PARSED: <comma-separated URLs>`.
...
- In the final chat response, include a line listing all external authority URLs that were successfully fetched during the investigation: `EXTERNAL URLS CHECKED: <comma-separated URLs>`, or `EXTERNAL URLS CHECKED: NONE` if no external fetches were performed.
